...
public class ReportCommand implements CommandExecutor, TabCompleter {
    ...
    public ReportCommand(CoreProtectionAssistant plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (args.length < 2) {
            player.sendMessage(Color.colorize(
                plugin.getConfigManager().getLangConfig().getString("messages.report_usage",
                    "%prefix% &cUsage: /report <player> <reason>")
            ));
            return true;
        }
        ...
        if (target == null || !target.isOnline()) {
            player.sendMessage(Color.colorize(
                plugin.getConfigManager().getLangConfig().getString("messages.player_offline",
                    "%prefix% &cPlayer is offline")
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (args.length == 2) {
            return List.of("Griefing", "Cheating", "Offensive language", "Spam", "Trolling");
        }
