...
public class ReportManager {
    ...
    public ReportResult createReport(Player reporter, Player target, String reason) {
        if (!enabled) {
        ...
        // Проверяем лимиты
        if (!checkReportLimits(reporter, target)) {
        ...
        if (reportId > 0) {
            ...
            // Проверяем на абуз
            checkReportAbuse(reporter, target);
            
            // Анализируем нарушителя
            analyzeViolator(target);
